...
class ModelRunner():
    ...
    def test(self, best_config):
        if best_config['gnorm']:
            X_train, Y_train, X_test, Y_test, mean_Y, std_Y, pre_X_test= self.data_preproc.get_train_test_samples(
                best_config['gnorm'])
        else:
            X_train, Y_train, X_test, Y_test, max_Y, pre_X_test = self.data_preproc.get_train_test_samples(best_config['gnorm'])
        ...
        Y_pred_training = model.sess.run([model.output], {model.X: X_train})


        if best_config['gnorm']:
            Y_pred_test = Y_pred_test * (std_Y + (std_Y == 0) * .001) + mean_Y
            Y_pred_training = Y_pred_training * (std_Y + (std_Y == 0) * .001) + mean_Y
        else:
            Y_pred_test = max_Y * Y_pred_test
            Y_pred_training = max_Y * Y_pred_training
        ...
        return Y_pred_test.ravel(), np.mean(rel_error) * 100, Y_test.ravel(), pre_X_test, Y_pred_training.ravel()
